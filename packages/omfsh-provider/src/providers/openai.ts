...
export function createOpenAIProvider(config: ProviderConfig): Provider {
  ...
  return {
    async generateText(prompt: string, options: ModelOptions): Promise<ProviderResponse<string>> {
    ...
    async streamText(prompt: string, options: ModelOptions): Promise<AsyncIterable<string>> {
    ...
    async embed(text: string, options: ModelOptions): Promise<ProviderResponse<number[]>> {
